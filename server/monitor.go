...
// DefaultGatewayAccountsListSize is the default size of a gateway's accounts list.
const DefaultGatewayAccountsListSize = 1024
...
type GatewayzOptions struct {
	...
	// AccountsLimit indicates how many accounts should be included in the results.
	// A default limit is used if not set.
	AccountsLimit int
}

// So that we can change during tests.
var gwAccountsLimit = DefaultGatewayAccountsListSize
...
func getMonitorGWOptions(opts *GatewayzOptions) (string, bool) {
	...
	if opts != nil {
		...
		accs = opts.Accounts
		if !accs && (opts.AccountName != _EMPTY_ || opts.AccountsLimit > 0) {
			accs = true
		}
	}
	return name, accs
...
// Returns options related to accounts.
func getMonitorGWAccOptions(opts *GatewayzOptions) (string, int) {
	var name string
	var limit int

	if opts != nil {
		name = opts.AccountName
		limit = opts.AccountsLimit
	}
	if limit <= 0 {
		limit = gwAccountsLimit
	}
	return name, limit
}
...
func createOutboundAccountsGatewayz(opts *GatewayzOptions, gw *gateway) []*AccountGatewayz {
	...
	accName, limit := getMonitorGWAccOptions(opts)
	if accName != _EMPTY_ {
	...
	gw.outsim.Range(func(k, v interface{}) bool {
		...
		accs = append(accs, a)
		// Return true to continue or false if limit is reached.
		return len(accs) < limit
	})
	return accs
}
...
func (s *Server) HandleGatewayz(w http.ResponseWriter, r *http.Request) {
	...
	accsLimit, err := decodeInt(w, r, "accs_limit")
	if err != nil {
		return
	}
